...
import { AccessControlEnumerableUpgradeable, AccessControlUpgradeable, UpgradeableBeacon } from "../../typechain-types";
import {
    AnyContractMeta,
    CONTRACTS,
    DEFAULT_ADMIN_ROLE,
    ...
    .setAction(async (taskArgs: { timelock: string }, hre) => {
        ...
        for (const contractMeta of Object.values(CONTRACTS)) {
            ...
            try {
                ...
                contract = anyContract as unknown as AccessControlUpgradeable;
            } catch (e) {
                validateError(e, "No Contract deployed with name");
                continue;
            }
...
task("revoke:admin", "revoke admin role").setAction(async (_taskArgs, hre) => {
    ...
    for (const contractMeta of Object.values(CONTRACTS)) {
        const name = contractMeta.name;
        let contract: AccessControlEnumerableUpgradeable;
        try {
            const anyContract = await getTypedContract(hre, contractMeta as AnyContractMeta);
            if (!anyContract.interface.hasFunction("DEFAULT_ADMIN_ROLE")) {
                continue;
            }
            contract = anyContract as unknown as AccessControlEnumerableUpgradeable;
        } catch (e) {
            validateError(e, "No Contract deployed with name");
            continue;
        }
